...
namespace NekkoOS
{
    ...
    public static unsafe class Boot
    {
        ...
        // ==========================================================
        // [UTILITY] Serial hex string input function
        // Allows users to enter hexadecimal values for RAM addresses or I/O port numbers
        // ==========================================================
        ...
        {
            byte* basePtr = (byte*)imageBase;
            int e_lfanew = *(int*)(basePtr + 0x3C);
            byte* nt = basePtr + e_lfanew;
            uint exportRVA = *(uint*)(nt + 136);
            if (exportRVA == 0) return null;

            byte* exportDir = basePtr + exportRVA;
            uint numberOfNames = *(uint*)(exportDir + 24);
            uint addressOfFunctionsRVA = *(uint*)(exportDir + 28);
            uint addressOfNamesRVA = *(uint*)(exportDir + 32);
            uint addressOfNameOrdinalsRVA = *(uint*)(exportDir + 36);

            uint* addressOfFunctions = (uint*)(basePtr + addressOfFunctionsRVA);
            uint* addressOfNames = (uint*)(basePtr + addressOfNamesRVA);
            ushort* addressOfNameOrdinals = (ushort*)(basePtr + addressOfNameOrdinalsRVA);

            for (uint i = 0; i < numberOfNames; i++) {
                char* name = (char*)(basePtr + addressOfNames[i]);
                if (IsKernelMain(name)) {
                    ushort ordinal = addressOfNameOrdinals[i];
                    uint funcRVA = addressOfFunctions[ordinal];
                    return (void*)(basePtr + funcRVA);
                }
            }
            return null;
        }
        ...
        public static long NekkoBoot(IntPtr imageHandle, EFI_SYSTEM_TABLE* systemTable)
        {
            ...
            else 
            {
                ...
                // ==========================================================
                // [RSA PUBLIC KEY] Standard C# array declaration
                // Empty declaration allows PowerShell build script to inject the actual key bytes
                // WARNING: Do not modify this line - build script expects this exact format!
                // ==========================================================
                byte* publicKeyN = stackalloc byte[256] {  }; /* INJECT_PUBKEY */
                ...
                if (relocRVA != 0) {
                    ...
                    while (bytesParsed < relocSize) {
                        ...
                        for (uint i = 0; i < relocEntriesCount; i++) {
                            ...
                            // ==========================================================
                            // [ĐỘNG CƠ RELOCATION] Xử lý 64-bit, 32-bit absolute và 32-bit RIP-relative
                            // Type 10 = IMAGE_REL_AMD64_ADDR64 (64-bit absolute)
                            // Type 3  = IMAGE_REL_AMD64_ADDR32 (32-bit absolute)
                            // Type 4  = IMAGE_REL_AMD64_REL32 (32-bit RIP-relative) - QUAN TRỌNG cho lea [rel symbol]!
                            // ==========================================================
                            if (type == 10) {
                                ...
                                // IMAGE_REL_AMD64_REL32: 32-bit RIP-relative
                                // Không cần adjust cho RIP-relative relocations vì chúng là offset trong cùng image
                                // (Địa chỉ tương đối không thay đổi khi image được relocate)
                            }
                        }
            ...
            fixed (char* dbg1 = "[BOOT] kernelBase = \0") Print(systemTable->ConOut, dbg1);
            ...
            fixed (char* dbg2 = "[BOOT] realEntry = \0") Print(systemTable->ConOut, dbg2);
            PrintHex(systemTable->ConOut, (ulong)realEntry);
            fixed (char* nl2 = "\r\n\0") Print(systemTable->ConOut, nl2);
            ...
            if (magicKey == 'D' || magicKey == 'd') {
                ...
                while (debugging) {
                    ...
                    char cmd = SerialReadChar(); // Wait for user command input
                    SerialWriteChar(cmd);        // Echo the typed character
                    fixed (char* nl = "\r\n\0") Print(systemTable->ConOut, nl);
